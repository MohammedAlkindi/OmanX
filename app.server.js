...
if (IS_PROD && !ADMIN_KEY) {
  logger.error("ADMIN_KEY is missing in production. Admin endpoints are disabled.");
}

if (IS_PROD && !ALLOWED_ORIGINS.length) {
  logger.warn("ALLOWED_ORIGINS is not set in production. Allowing all origins until configured.");
}
...
function requireAdmin(req, res, next) {
  if (!ADMIN_ENABLED) {
    return res.status(503).json({ error: "Admin endpoints are disabled", requestId: req.requestId });
  }
